...
const StaffPage = () => {
  ...
  // --- FETCH STAFF (Agora traz Professores e Staff) ---
  ...
  const handleSaveProfile = () => {
    ...
    // TODO: Adicionar lógica de PUT para o backend aqui na Fase 4
    setStaffList(staffList.map(s => s.id === selectedStaff.id ? editProfileData : s));
    setSelectedStaff(editProfileData);
    setIsEditingProfile(false);
  };

  // --- FUNÇÃO ELIMINAR ATUALIZADA ---
  const handleDeleteStaff = async () => {
    if (!selectedStaff) return;

    try {
      // ATENÇÃO: Agora enviamos o role para o backend saber em que tabela apagar
      ...
      alert("Erro ao eliminar. Verifique se o backend foi atualizado.");
    }
  };

  const handleCreateSubmit = () => {
    console.log("Dados a enviar:", formData);
    // TODO: Adicionar lógica de POST aqui na Fase 4
